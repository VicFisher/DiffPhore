...
def modify_conformer(data, tr_update, rot_update, torsion_updates, keep_update=False):
    ...
    Args:
        data (torch_geometric.data.HeteroData): The hetero-graphs.
...
def set_time(complex_graphs, t_tr, t_rot, t_tor, batchsize, all_atoms, device):
    """
    Sets the time attributes for the nodes in the ligand, receptor, and optionally atom graphs within the complex graphs.

    Args:
    complex_graphs (HeteroDataBatch): HeteroData graphs for 'ligand', 'receptor', and optionally 'atom'.
...
def set_time_phore(graphs, t_tr, t_rot, t_tor, batchsize, device):
    ...
    Args:
    graphs (HeteroDataBatch): ligand-pharmacophore hetero-graphs.
