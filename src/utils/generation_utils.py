"""
This module provides utility functions for molecular generation and optimization mostly for TANKBind.
"""
...
def distance_loss_function(epoch, y_pred, x, protein_nodes_xyz, compound_pair_dis_constraint, LAS_distance_constraint_mask=None, mode=0):
    dis = torch.cdist(protein_nodes_xyz, x).flatten().to(x.device)
    dis_clamp = torch.clamp(dis, max=5)
    if mode == 0:
...
def distance_optimize_compound_coords(coords, y_pred, protein_nodes_xyz, 
                        compound_pair_dis_constraint, total_epoch=5000, loss_function=distance_loss_function, 
                        LAS_distance_constraint_mask=None, mode=0, show_progress=False,
                        device='cuda'):
    # random initialization. center at the protein center.
    c_pred = protein_nodes_xyz.mean(axis=0).to(device).reshape(1, 3)
    x = (5 * (2 * torch.rand(coords.shape).to(device) - 1) + c_pred)
...
def get_info_pred_distance(coords, y_pred, protein_nodes_xyz, compound_pair_dis_constraint, 
                           n_repeat=1, LAS_distance_constraint_mask=None, mode=0, show_progress=False, 
                           device='cuda'):
    LAS_distance_constraint_mask = LAS_distance_constraint_mask if LAS_distance_constraint_mask is None \
        else LAS_distance_constraint_mask
    info = []
    if show_progress:
    ...
    for repeat in it:
        ...
        x, loss_list, rmsd_list = distance_optimize_compound_coords(coords, y_pred, protein_nodes_xyz, 
                                                                    compound_pair_dis_constraint, 
                                                                    LAS_distance_constraint_mask=LAS_distance_constraint_mask, 
                                                                    mode=mode, show_progress=True, device=device)
...
def get_predict_results(model, graph, remove_hs=True, show_progress=False, device='cuda'):
    y_pred, affinity_pred = model(graph)
    mol = graph.mol[0] if isinstance(graph.mol, list) else graph.mol
    _mol = copy.deepcopy(mol)
    _mol = Chem.RemoveHs(_mol, sanitize=True) if remove_hs else _mol
    compound_pair_dis_constraint = torch.cdist(graph['ligand'].pos, graph['ligand'].pos).to(device)
    LAS_distance_constraint_mask = get_LAS_distance_constraint_mask(_mol).bool().to(device)
    lig_orig_pos = graph['ligand'].orig_pos[0] if isinstance(graph['ligand'].orig_pos, list) else graph['ligand'].orig_pos
    lig_orig_pos = torch.tensor(lig_orig_pos, dtype=torch.float, device=device)
    y_pred = y_pred.to(device)
    pred_dist_info = get_info_pred_distance(lig_orig_pos, y_pred, graph['phore'].pos, compound_pair_dis_constraint,
                                            LAS_distance_constraint_mask=LAS_distance_constraint_mask,
                                            n_repeat=1, show_progress=show_progress, device=device)
    # pred_dist_info = get_info_pred_distance(graph['ligand'].pos, y_pred, graph['phore'].pos, compound_pair_dis_constraint,
    #                                         LAS_distance_constraint_mask=LAS_distance_constraint_mask,
    #                                         n_repeat=1, show_progress=show_progress, device=device)
    return pred_dist_info
