...
def target_fishing_run(baseline, drug, smiles="",
    ...
    if num_workers > 1:
        df_pdbids = pd.DataFrame({'pdbid': os.listdir(dataset_dir)})
        if baseline == 'unidock':
            df_pdbids['result_file'] = df_pdbids['pdbid'].map(lambda x: os.path.join(output_dir, x, f"{drug}_uni.sdf"))
        df_pdbids.parallel_apply(lambda x: run_docking(x['pdbid'], baseline, dataset_dir, 
                                                       ...
                                                       num_flag=num_flag, result_file=x['result_file'] if baseline == 'unidock' else ""), axis=1)
    else:
        for pdb in os.listdir(dataset_dir):
            run_docking(pdb, baseline, dataset_dir, output_dir)
...
def run_docking(pdb, baseline, dataset_dir, output_dir, ligand_file="", size_flag="", cpu_flag="", num_flag="", result_file=""):
    try:
        tool = tools[baseline]
        ...
        status = "1"
        if baseline == 'vina':
            result_file = os.path.join(output_pdb, f"{pdb}.pdbqt") 
        elif baseline in ['smina', 'gnina']:
            result_file = os.path.join(output_pdb, f"{pdb}.sdf")

        if os.path.exists(status_file):
            with open(status_file, 'r') as fstat:
                status = fstat.readline().strip()

        if status == "0" and (result_file != "" and not os.path.exists(result_file)):
            status = "1"
            print(f"[W] {baseline} for `{pdb}`. Status shows runned, but result file not exists. Redocking.")
