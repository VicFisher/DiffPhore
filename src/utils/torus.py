...
if os.path.exists(os.path.join(CACHE_PATH, '.p.npy')):
    p_ = np.load(os.path.join(CACHE_PATH, '.p.npy'))
    score_ = np.load(os.path.join(CACHE_PATH, '.score.npy'))
else:
    print("Precomputing and saving to cache torus distribution table")
    p_ = p(x, sigma[:, None], N=100)
    np.save(os.path.join(CACHE_PATH, '.p.npy'), p_)

    score_ = grad(x, sigma[:, None], N=100) / p_
    np.save(os.path.join(CACHE_PATH, '.score.npy'), score_)
