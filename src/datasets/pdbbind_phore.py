...
def construct_dataset(args):
    # tank_args = {'remove_hs':args.remove_hs, 'use_LAS_constrains':args.use_las_constrains, 'sanitize': True} if args.model_type == 'tank' else None
    # transform = NoiseTransformPhore(t_to_sigma=t_to_sigma, no_torsion=args.no_torsion, tank_args=tank_args)
    common_args = {'transform': None, 'root': args.data_dir, 'limit_complexes': args.limit_complexes,
                   'remove_hs': args.remove_hs, 'max_lig_size': args.max_lig_size, 'matching': not args.no_torsion, 
                   'popsize': args.matching_popsize, 'maxiter': args.matching_maxiter, 'consider_ex': args.consider_ex,
                   'neighbor_cutoff': args.neighbor_cutoff, 'ex_connected': args.ex_connected, 'num_workers': args.num_workers,
                   'use_LAS_constrains':args.use_las_constrains, 'tank_features': args.model_type == 'tank', 'dataset': args.dataset,
                   'use_phore_rule': False, 'use_sdf': args.use_sdf, 'ro5_filter': args.ro5_filter, 'near_phore': args.near_phore}
    ...
    train_graphs = copy.deepcopy(train_dataset.graphs)
    num = 0
    for idx in range(len(train_graphs)):
        train_graphs[idx].mol = copy.deepcopy(train_dataset.rdkit_ligands[idx])
        if not hasattr(train_graphs[idx], 'phorefp'):
            generate_ligand_phore_feat(train_graphs[idx].mol, train_graphs[idx], remove_hs=train_dataset.remove_hs)
        if not hasattr(train_graphs[idx], 'phoretype'):
            train_graphs[idx]['phore'].phoretype = torch.tensor([[int(x) == y for y in range(11)] for x in train_graphs[idx]['phore'].x[:, 0]]).float()
        num += 1
        if num % 200 == 0: 
            print(f"{num} train samples proccessed")
    
    torch.save(train_graphs, '/home/kg108/YJL/DiffPhore/data/cache_torsion/train_heterographs.pkl')
    torch.save(train_dataset.rdkit_ligands, '/home/kg108/YJL/DiffPhore/data/cache_torsion/train_rdkit_ligands.pkl')
    print("Saved train graphs to: /home/kg108/YJL/DiffPhore/data/cache_torsion/train_heterographs.pkl ")
    print("Saved train rdkit_ligands to: /home/kg108/YJL/DiffPhore/data/cache_torsion/train_rdkit_ligands.pkl ")
    
    num = 0
    val_graphs = copy.deepcopy(val_dataset.graphs)
    for idx in range(len(val_graphs)):
        val_graphs[idx].mol = copy.deepcopy(val_dataset.rdkit_ligands[idx])
        if not hasattr(val_graphs[idx], 'phorefp'):
            generate_ligand_phore_feat(val_graphs[idx].mol, val_graphs[idx], remove_hs=val_dataset.remove_hs)
        if not hasattr(val_graphs[idx], 'phoretype'):
            val_graphs[idx]['phore'].phoretype = torch.tensor([[int(x) == y for y in range(11)] for x in val_graphs[idx]['phore'].x[:, 0]]).float()
        num += 1
        if num % 200 == 0: 
            print(f"{num} val samples proccessed")
    torch.save(val_graphs, '/home/kg108/YJL/DiffPhore/data/cache_torsion/val_heterographs.pkl')
    torch.save(val_dataset.rdkit_ligands, '/home/kg108/YJL/DiffPhore/data/cache_torsion/val_rdkit_ligands.pkl')
    print("Saved valid graphs to: /home/kg108/YJL/DiffPhore/data/cache_torsion/val_heterographs.pkl")
    print("Saved valid rdkit_ligands to: /home/kg108/YJL/DiffPhore/data/cache_torsion/val_rdkit_ligands.pkl")
...
def calc_pdbbind_fitscore(pdbid, out_path, pdbbind_path="/home/worker/users/YJL/DiffPhore/data/PDBBind", dedup=False):
