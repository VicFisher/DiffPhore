...
CACHE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../../data/cache_diffusion")
# print(f"CACHE_PATH: {CACHE_PATH}")
if os.path.exists(os.path.join(CACHE_PATH, '.so3_omegas_array2.npy')):
    _omegas_array = np.load(os.path.join(CACHE_PATH, '.so3_omegas_array2.npy'))
    _cdf_vals = np.load(os.path.join(CACHE_PATH, '.so3_cdf_vals2.npy'))
    _score_norms = np.load(os.path.join(CACHE_PATH, '.so3_score_norms2.npy'))
    _exp_score_norms = np.load(os.path.join(CACHE_PATH, '.so3_exp_score_norms2.npy'))
else:
    ...
    np.save(os.path.join(CACHE_PATH, '.so3_omegas_array2.npy'), _omegas_array)
    np.save(os.path.join(CACHE_PATH, '.so3_cdf_vals2.npy'), _cdf_vals)
    np.save(os.path.join(CACHE_PATH, '.so3_score_norms2.npy'), _score_norms)
    np.save(os.path.join(CACHE_PATH, '.so3_exp_score_norms2.npy'), _exp_score_norms)
