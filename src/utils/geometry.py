...
def rigid_transform_Kabsch_3D_torch(A, B):
    """
    Computes the optimal rigid transformation (rotation and translation) that aligns
    two sets of 3D points using the Kabsch algorithm.

    Args:
        A (torch.Tensor): A 3xN matrix of 3D points.
        B (torch.Tensor): A 3xN matrix of 3D points.

    Returns:
        tuple: A tuple (R, t) where R is a 3x3 rotation matrix and t is a 3x1 translation vector.

    Raises:
        Exception: If the input matrices A or B are not of shape 3xN.
    """
