...
def get_lig_graph(mol, complex_graph):
    ...
    Args:
        ...
        complex_graph (torch_geometric.data.HeteroData): The complex graph to be updated with ligand information.
...
def get_lig_graph_with_matching(mol_, complex_graph, popsize, maxiter, matching, keep_original, num_conformers, remove_hs):
    ...
    complex_graph (torch_geometric.data.HeteroData): The complex graph to update with ligand information.
...
def generate_ligand_phore_feat(mol, data=None, remove_hs=True):
    ...
    data (torch_geometric.data.HeteroData, optional): the ligand graph. Defaults to None.
    ...
    - The function updates the `data` with the generated features if `data` is provided.
    """
