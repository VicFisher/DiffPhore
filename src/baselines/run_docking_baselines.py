...
def docking_run(baseline, dataset, size=20, cpu=8, num_modes=10, num_workers=1,
    ...
    """
    Running the molecular docking for a whole dataset
    Args:
        baseline: The docking tools to run out of ['vina', 'smina', 'gnina', 'unidock'].
        dataset: The dataset to be evaluated on. Out of ['pdbbind', 'pdbbind'].
        size: The size of the pocket.
        cpu: The cpu option needs to be specified for ['vina', 'smina', 'gnina'].
        num_modes: The number of binding modes to be generated.
        num_workers: The number of workers to run docking.
        prepared_path: The path stored the prepared input files for docking.
        output_path: The path to store the docking poses and logs.
    """
...
def target_fishing_run(baseline, drug, smiles="",
        ...
        output_path="/home/worker/users/YJL/DiffPhore/experiments/baselines/output"):
    """
    Running the target fishing for a drug
    Args:
        baseline: The docking tools to run, out of ['vina', 'smina', 'gnina', 'unidock'].
        drug: The drug name to conduct target fishing.
        dataset: target fishing task
        size: The size of the pocket.
        cpu: The cpu option needs to be specified for ['vina', 'smina', 'gnina'].
        num_modes: The number of binding modes to be generated.
        num_workers: The number of workers to run target fishing.
        prepared_path: The path stored the prepared input files for target fishing.
        drug_path: The cache path of the drug file
        output_path: The path to store the target fishing poses and logs.
    """
...
def virtual_screening_run(baseline, target, 
        ...
        output_path="/home/worker/users/YJL/DiffPhore/experiments/baselines/output"):
    """
    Running virtual screening for a target
    Args:
        baseline: The docking tools to run, out of ['vina', 'smina', 'gnina', 'unidock'].
        target: The target to carry out virtual screening.
        size: The size of the pocket.
        cpu: The cpu option needs to be specified for ['vina', 'smina', 'gnina'].
        num_modes: The number of binding modes to be generated.
        num_workers: The number of workers to run target fishing.
...
def run_docking(pdb, baseline, dataset_dir, output_dir, ligand_file="", size_flag="", cpu_flag="", num_flag=""):
    """
    Molecular docking running functionality.
    Args:
        pdb: The pdb id of current input.
        baseline: The docking tool.
        dataset_dir: The directory of the dataset.
        output_dir: The directory to store the output files.
        ligand_file: The ligand file to locate the binding site.
        size_flag: The option string to indicate the docking range.
        cpu_flag: The option string to specify the number of CPU to use.
        num_flag: The option string to specify the number of modes to generate.
    """
    try:
        tool = tools[baseline]
...
def run_docking_vs(target, name, baseline, dataset_dir, output_dir, ligand_file="", size_flag="", cpu_flag="", num_flag=""):
    """
    Docking-driven virtual screening for a specified target.
    """
    try:
        tool = tools[baseline]
        ...
        print(f"[E] {baseline} failed for `{target}`. {e}")
...
def json_to_config_flag(json_file, ligand_file=""):
    """
    Convert the config file to command line flags
    """
...
def select_ligand_file(drug, baseline, smiles="", prepared=False,
                       drug_path="/home/worker/users/YJL/DiffPhore/experiments/diffphore_inference/target_fishing/drugs/"):
    """
    Choose corresponding ligand file for each baseline.
    """
    try:
        if not prepared:
