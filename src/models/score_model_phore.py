...
# def fully_connect_two_graphs(batch_1, batch_2, mask_1=None, mask_2=None):
...
#     new_index = []
#     # new_index = torch.empty(2, 0).to(batch_1.device)

#     for i in range(len(masked_index_1)):
#         for j in range(len(masked_index_2)):
#             if masked_batch_1[i] == masked_batch_2[j]:
#                 index = [masked_index_1[i], masked_index_2[j]]
#                 new_index.append(index)
#                 # new_index = torch.concat([new_index, torch.tensor(
#                 #     [[masked_index_1[i]], [masked_index_2[j]]]).to(batch_1.device)], axis=1)
#     new_index = torch.tensor(new_index).to(batch_1.device).long().T
#     return new_index
