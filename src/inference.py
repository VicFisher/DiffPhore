...
def str2bool(inp):
    """
    Transform the strings from command line to bool type
    """
    inp = inp.lower()
...
def read_input(phore_ligand_csv=None, phore=None, ligand=None):
    """
    Preprocess the input of DiffPhore into unified List of Dict format.
    Args:
        phore_ligand_csv: path to the csv format file with fields of `phore` and `ligand_description`.
        phore: path to a single pharmacophore file ends with suffix '.phore'.
        ligand: the description of ligand in the format of SMILES or SDF/MOL files.
    
    Return:
        records, List[Dict], the unified input format
    """
    records = []
...
def fit(args, model, complex_graphs, device, t_to_sigma, tmp_log="", n_report=1000):
    """
    The main ligand-pharmacophore fitting functionality. 
    Args:
        args: The fitting settings.
        model: DiffPhore model.
        complex_graphs: The processed input in heterograph format.
        device: The device to conduct the fitting.
        t_to_sigma: The function to obtain noise sigma from predefined noise schedule given time.
        tmp_log: The path to the tmp file to check the result of last finished batch.
        n_report: The number of batches to report the fitting process.
    
    Return:
        metrics: The metrics of fitting results    
    """
...
def get_perfect_similarity(g, weights=[1, 1, 1, 1, 1, 0, 1, 1, 1, 1, 0], 
                        ...
                        debug=False):
    """
    Pharmacophore fingerprint similarity only taking type and count into consideration. Regardless of actual fitting.
    Args:
        g: Heterograph consisting of pharmacophore and ligand
        weights: The weights of different types of pharmacophores.
        alpha: The tolerance of different types of pharmacophores.
        debug: Whether to show the intermediate variables for debugging.
    """
...
def my_max(x):
    """
    Customized max function to process the fitness scores.
    """
...
def main():
    """
    Ligand-pharmacophore fitting powered by DiffPhore
    """
